...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!apiKey) {
    return res.status(500).json({
      error: 'Email is not configured. Set BREVO_API_KEY in Vercel.',
    })
  }
  ...
  try {
    const admin = await sendBrevoEmail({
      apiKey,
      from: fromEmail,
      to: toEmail,
      replyTo: email,
    ...
    console.info('Admin email sent', { to: toEmail, from: fromEmail, messageId: admin.messageId })
  } catch (error) {
    const messageText = error instanceof Error ? error.message : 'Unknown Brevo error'
    console.error('Admin email failed', { message: messageText, from: fromEmail, to: toEmail })
    return res.status(502).json({ error: formatBrevoError(messageText) })
  }
  ...
  try {
    ...
    const messageText = error instanceof Error ? error.message : 'Unknown Brevo error'
    console.error('Confirmation email failed', { message: messageText, to: email, from: fromEmail })
  }
